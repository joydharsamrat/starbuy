            ...
            <div className='bg-gray-300 dark:bg-gray-600 p-10 rounded-xl shadow-lg'>
                ...
                <form className='mt-10' onSubmit={handleSubmit(handelAddProduct)}>
                    <div className="form-control w-full  my-3">
                        <input {...register("name", { required: "Name is required" })} type="text" placeholder='Product Name' className="input input-bordered w-full " />
                        {errors.name && <p className='text-red-600'>{errors.name?.message}</p>}
                    </div>
                    <div className="form-control w-full  my-3">
                        <input {...register("price", { required: "price is required" })} type="text" placeholder='Price' className="input input-bordered w-full " />
                        {errors.price && <p className='text-red-600'>{errors.price?.message}</p>}
                    </div>

                    <div className="form-control w-full  my-3">
                        <label className='dark:text-white ' htmlFor="photo">Product Photo</label>
                        <input id='photo' type="file"{...register("photo", { required: "photo is required" })} placeholder="Your Photo" className="file-input w-full  mb-3" />
